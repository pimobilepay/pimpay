...
export default function SendModal({ isOpen, onClose, balance, onRefresh }: any) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md">
      <div className="bg-[#0a0a0a] w-full max-w-xs rounded-[2.5rem] border border-white/10 p-8 relative shadow-2xl">
        ...
        <h4 className="text-lg font-black text-white mb-6 uppercase text-center tracking-tighter">Envoyer SDA</h4>
        ...
        {status === "success" ? (
          <div className="flex flex-col items-center py-4 text-center">
            ...
            <p className="text-[10px] text-slate-400 mb-6 px-4 font-medium uppercase tracking-tight">
              {amount} SDA ont été envoyés avec succès.
            </p>

            <div className="w-full space-y-3 mb-8">
              {/* Détails clairs pour éviter toute confusion */}
              ...
              {/* LIEN VERS L'EXPLORATEUR SIDRA */}
              <a 
                href={`https://ledger.sidrachain.com/tx/${txHash}`} 
                target="_blank" 
                rel="noopener noreferrer"
                className="flex items-center justify-between bg-white/5 border border-white/10 p-3 rounded-2xl text-left hover:bg-white/10 transition-colors"
              >
                <div>
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 uppercase ml-2">Adresse de destination</label>
              <input
                value={address}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-500 uppercase ml-2 flex justify-between">
                <span>Montant</span>
                <span className="text-blue-500 font-bold">Solde: {balance}</span>
              </label>
              <div className="relative">
                ...
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-600 uppercase">SDA</span>
              </div>
            </div>

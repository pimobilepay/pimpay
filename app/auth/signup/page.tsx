...
import { Eye, EyeOff, User, Mail, Phone, Lock, ShieldCheck, CheckCircle2 } from "lucide-react";
...
export default function SignupPage() {
  ...
  const [formData, setFormData] = useState({
    fullName: "",
    username: "", // Ajouté pour correspondre à votre API signup
    email: "",
    phone: "",
    password: "",
    confirmPassword: "",
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // On génère un username à partir de l'email si vide
      const signupData = {
        ...formData,
      ...
      // Stockage du token pour l'étape suivante (PIN)
      if (data.token) {
        setToken(data.token);
  ...
  const handleSetPin = async () => {
    ...
    // Récupération du token depuis l'état ou le stockage local
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setStep(3);
        localStorage.removeItem("pimpay_token"); // Nettoyage
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-4 relative overflow-hidden text-white">
      ...
      <div className="mb-6 text-center relative z-10">
        <h1 className="text-4xl font-black italic tracking-tighter text-white">
          PIMPAY<span className="text-blue-500 font-normal">.</span>
        </h1>
      </div>
      ...
      <Card className="w-full max-w-md p-8 bg-slate-900/50 backdrop-blur-2xl border-white/10 shadow-2xl relative z-10 rounded-[32px]">
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-1 text-white">Créer un compte</h2>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    className="h-12 pl-12 bg-slate-950/50 border-white/10 rounded-xl focus:border-blue-500 text-white placeholder:text-slate-600"
              ...
              <Button type="submit" className="w-full h-14 bg-blue-600 hover:bg-blue-500 rounded-2xl font-bold shadow-lg shadow-blue-600/20 active:scale-[0.98] text-white" disabled={loading}>
        ...
        {step === 2 && (
          <div className="space-y-8 py-4 text-center">
            ...
            <div className="flex justify-center gap-4 relative">
               {[0, 1, 2, 3].map((i) => (
                 <div key={i} className={`w-12 h-16 rounded-xl border-2 flex items-center justify-center text-2xl font-bold transition-all ${pin[i] ? "border-blue-500 bg-blue-500/10 text-white" : "border-white/10 bg-slate-950/50 text-slate-700"}`}>
                   {pin[i] ? "•" : "0"}
                 </div>
               ))}
               <input
            ...
            <Button onClick={handleSetPin} className="w-full h-14 bg-blue-600 rounded-2xl font-bold text-white disabled:opacity-30" disabled={pin.length !== 4 || loading}>
              {loading ? "Configuration..." : "Confirmer le code PIN"}
            </Button>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-8 py-6 text-center">
             ...
             <div>
                <h2 className="text-2xl font-bold mb-2 text-white">Bienvenue à bord !</h2>

...
import {
  ...
  LayoutGrid, Receipt, Smartphone, Share2, User,
  TrendingUp
} from "lucide-react";
...
export default function UserDashboard() {
  ...
  // 1. Correction majeure : On ne rend rien avant le montage pour éviter l'erreur removeChild
  if (!hasMounted) {
    ...
    <div className="min-h-screen bg-[#020617] text-white pb-32 font-sans">
      ...
      <header className="px-6 py-6 flex justify-between items-center bg-[#020617]/80 backdrop-blur-md sticky top-0 z-50 border-b border-white/5">
        ...
        <div className="flex gap-2">
            <button onClick={() => router.push("/profile")} className="p-3 rounded-2xl bg-white/5 text-slate-400 hover:text-blue-400 transition-colors">
              <User size={20} />
            </button>
      ...
      <main className="px-6">
          ...
          <div className="relative z-10 h-full flex flex-col justify-between">
            ...
            <div className="flex justify-between items-end">
                <div className="relative">
                    ...
                    {showCurrencyPicker && (
                        <div className="absolute bottom-full left-0 mb-2 w-28 bg-[#0f172a]/95 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl z-[60]">
        ...
        <div className="grid grid-cols-4 gap-4 mb-10">
            {[
              ...
              <button key={`action-${i}`} onClick={() => router.push(action.link)} className="flex flex-col items-center gap-2">
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-4 gap-4">
              {[
                ...
                <button key={`quick-${i}`} className="flex flex-col items-center gap-3 group">
        ...
        {/* STATISTIQUE GRAPHIQUE - Sécurisé pour le montage */}
        <section className="mb-10 p-6 bg-slate-900/40 border border-white/5 rounded-[32px]">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Activity Analytics</h3>
              <p className="text-lg font-black italic">+12.5% <span className="text-blue-500 text-xs">this week</span></p>
            </div>
          ...
          <div className="h-40 w-full">
            {/* On rend le ResponsiveContainer uniquement sur le client */}
        ...
        <section className="mb-8">
          ...
          <div className="space-y-3">
            {transactions.length > 0 ? transactions.map((tx: any) => (
              <div key={tx.id || Math.random()} className="p-4 bg-slate-900/30 border border-white/5 rounded-[24px] flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">{new Date(tx.createdAt).toLocaleDateString('fr-FR')}</p>
                  </div>
                </div>
                <div className="text-right">
      ...
      <BottomNav onOpenMenu={toggleSidebar} />
    </div>
  );
}

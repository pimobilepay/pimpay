...
export default function UserDashboard() {
  ...
  async function fetchMarketPrices() {
    try {
      const res = await fetch(
        "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,tether,usd-coin,dai,binance-usd,ripple,stellar&vs_currencies=usd"
      );
      if (res.ok) {
  ...
  // ----------------------------
  // DERIVED (safe even if loading)
  // ----------------------------
  const wallets = data?.wallets || [];
  const currentWallet =
    wallets[activeWalletIndex] || ({ id: undefined, balance: 0, currency: "PI" } as any);

  const balance = currentWallet.balance || 0;
  const currentCurrency = currentWallet.currency || "PI";
  ...
  // ----------------------------
  // CHART FIX (Prisma-aligned)
  // - No "Total" slice in Pie
  // - Wallet-aware stats using fromWalletId/toWalletId
  // - Works even if API no longer returns tx.isDebit
  // ----------------------------
  const computeStats = (txsRaw: any[], walletId?: string, userId?: string) => {
    const txsSafe = Array.isArray(txsRaw) ? txsRaw : [];

    // focus stats on active wallet if possible (Prisma: fromWalletId / toWalletId)
    const scopedTxs = walletId
      ? txsSafe.filter((t) => t?.fromWalletId === walletId || t?.toWalletId === walletId)
      : txsSafe;

    const isDebit = (t: any) => {
      // Respect API field if present
      if (typeof t?.isDebit === "boolean") return t.isDebit;

      // Best: derive from wallet relation
      if (walletId) return t?.fromWalletId === walletId;

      // Fallback: derive from user relation
      if (userId) return t?.fromUserId === userId;

      return false;
    };

    const sent = scopedTxs.filter((t) => isDebit(t) && t?.type !== "EXCHANGE").length;
    const received = scopedTxs.filter((t) => !isDebit(t) && t?.type !== "EXCHANGE").length;
    const swaps = scopedTxs.filter((t) => t?.type === "EXCHANGE").length;
    const total = scopedTxs.length;
    ...
    const list = [...pie, { name: "Total", value: total || 0 }];

    return { pie, list, total };
  };

  const stats = computeStats(data?.transactions || [], currentWallet?.id, data?.id);
  ...
  const getTxIcon = (tx: any) => {
    if (tx.type === "EXCHANGE")
      return { icon: <RefreshCcw size={18} />, color: "bg-orange-500/10 text-orange-500" };
    if (!tx.isDebit)
      return { icon: <ArrowDownCircle size={18} />, color: "bg-emerald-500/10 text-emerald-500" };
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans flex flex-col">
      ...
      <header className="px-6 py-6 flex justify-between items-center bg-[#020617]/80 backdrop-blur-md sticky top-0 z-[100] border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-blue-400 uppercase tracking-[0.2em] mt-1">
              Virtual Bank
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="p-3 rounded-2xl bg-white/5 text-slate-400"
            >
              <User size={20} />
            </button>
            ...
            {showProfileMenu && (
              <div className="absolute right-0 mt-3 w-56 bg-slate-900 border border-white/10 rounded-[24px] shadow-2xl p-2 z-[110]">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 text-xs font-bold uppercase text-left"
                >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-xl text-rose-500 text-xs font-bold uppercase text-left"
                >
                    ...
                    <div className="absolute right-0 mt-2 w-40 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-1 z-[120]">
                      {wallets.map((w: any, idx: number) => (
                        <button
                          ...
                          className={`w-full flex items-center justify-between p-3 rounded-xl text-[10px] font-black uppercase tracking-widest ${
                        ...
                        >
                          ...
                          <span className="opacity-60 text-[8px]">
                            {showBalance ? w.balance.toFixed(2) : "••"}
                          </span>
                        </button>
                      ))}
                    </div>
            ...
            <div>
              <h2 className="text-4xl font-black tracking-tighter flex items-center gap-2">
                <span className="text-blue-200">
                  {currentCurrency === "PI" ? "π" : currentCurrency === "XAF" ? "" : "$"}
                </span>
                {showBalance
                  ? balance.toLocaleString("fr-FR", { minimumFractionDigits: 2 })
                  : "••••••"}
              </h2>
              <p className="text-[10px] text-white/60 font-black uppercase tracking-[0.2em] mt-1">
                {displayName}
              </p>
            </div>
            ...
            <div className="flex justify-between items-end">
              <div className="bg-black/30 px-3 py-1.5 rounded-xl border border-white/10 flex items-center gap-2">
                ...
                <p className="text-[11px] font-mono font-bold">
                  ≈ {showBalance ? `${convertedValue.toLocaleString()} ${currency}` : "Locked"}
                </p>
              </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            ...
            <button key={i} onClick={() => router.push(action.link)} className="flex flex-col items-center gap-2">
              <div
                className={`${action.color} w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg active:scale-95 transition-transform`}
              >
                {action.icon}
              </div>
            ...
            <div className="w-32 h-32 relative">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={
                      stats.pie.filter((s) => s.value > 0).length > 0
                        ? stats.pie.filter((s) => s.value > 0)
                        : [{ name: "Aucune", value: 1 }]
                    }
                    cx="50%"
                    cy="50%"
                  ...
                  >
                    {stats.pie.filter((s) => s.value > 0).length > 0
                      ? stats.pie
                          .filter((s) => s.value > 0)
                          .map((entry, index) => {
                            const colorIndex = stats.pie.findIndex((s) => s.name === entry.name);
                            return (
                              <Cell
                                key={index}
        ...
        <section className="mb-12 relative">
          ...
          <div className="space-y-4 relative z-10 max-h-[400px] overflow-y-auto no-scrollbar pr-1">
            {data?.transactions?.length > 0 ? (
              ...
              <div className="text-center py-10 opacity-30 text-[10px] uppercase font-bold border border-dashed border-white/10 rounded-[32px]">
                Aucune transaction
              </div>
            )}
          </div>
      ...
      <footer className="pt-8 pb-32 border-t border-white/5 flex flex-col items-center gap-6 bg-[#020617]">
        <div className="flex items-center gap-6">
          <a
            href="https://www.facebook.com/profile.php?id=61583243122633"
            className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-blue-500 transition-all"
          >
            <Facebook size={20} />
          </a>
          <a
          ...
          <a
            ...
            className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-red-500 transition-all"
          >
            <Youtube size={20} />
          </a>
        </div>

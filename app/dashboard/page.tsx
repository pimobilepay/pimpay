...
export default function UserDashboard() {
  ...
  // [FIX] Soldes fiat (USD/XOF/EUR/XAF) — auparavant absents du dashboard.
  const [fiatBalances, setFiatBalances] = useState<Record<string, number>>({ USD: 0, XOF: 0, EUR: 0, XAF: 0 });
  ...
  async function fetchDashboardData() {
    ...
    try {
      ...
      if (balRes.ok) {
        ...
        // il n'entrait ni dans le total affiché en haut du dashboard, ni
        // dans la liste des wallets. Il est désormais extrait de la liste
        // complète `wallets` renvoyée par /api/wallet/balance.
        const fiat: Record<string, number> = { USD: 0, XOF: 0, EUR: 0, XAF: 0 };
        if (Array.isArray(balData.wallets)) {
          for (const w of balData.wallets) {
            if (w.currency in fiat) fiat[w.currency] = w.balance || 0;
          }
        }
        // XAF est aussi exposé directement à la racine par l'API.
        fiat.XAF = fiat.XAF || parseFloat(balData.XAF || "0") || 0;
        setFiatBalances(fiat);
      }
    } catch {
  ...
  // [FIX] Valeur des soldes FIAT convertie en USD puis ajoutée au total,
  // exactement comme les cryptos ci-dessus. XOF/XAF sont pegged à
  // 655.957 pour 1 EUR (union monétaire CFA) ; EUR converti en USD via
  // le taux USD_TO_XAF déjà utilisé pour tout le dashboard.
  const XAF_PER_EUR = 655.957;
  const fiatUsdValue =
    (fiatBalances.USD || 0) +
    (fiatBalances.EUR || 0) * (XAF_PER_EUR / USD_TO_XAF) +
    (fiatBalances.XOF || 0) / USD_TO_XAF +
    (fiatBalances.XAF || 0) / USD_TO_XAF;

  const totalInXAF = (totalUSDValue + fiatUsdValue) * USD_TO_XAF;
  ...
  // [FIX] Les wallets FIAT (USD/XOF/EUR/XAF) sont désormais inclus ici, avec
  // le même taux de conversion utilisé pour le total (voir fiatUsdValue
  // ci-dessus) — ils apparaissent maintenant dans les "wallets" du dashboard
  // au lieu d'être invisibles.
  const FIAT_USD_RATE: Record<string, number> = {
    USD: 1,
    EUR: XAF_PER_EUR / USD_TO_XAF,
    XOF: 1 / USD_TO_XAF,
    XAF: 1 / USD_TO_XAF,
  };
  const topWallets = [
    ...Object.entries(balances).map(([k, v]) => ({ currency: k, balance: v, usdValue: v * (marketPrices[k] || 0) })),
    ...Object.entries(fiatBalances).map(([k, v]) => ({ currency: k, balance: v, usdValue: v * (FIAT_USD_RATE[k] || 0) })),
  ]

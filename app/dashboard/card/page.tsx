...
// CONFIGURATION DES PALIERS (Prix USD, Limites, Années)
const CARD_TIERS = {
  PLATINIUM: {
    label: "Platinium Visa",
    price: 10,
    daily: 1000,
    recharge: 10000,
    years: 3,
  ...
  ULTRA: {
    label: "MCard Ultra",
    price: 100,
    daily: 999999, // Illimité
    recharge: 999999, // Illimité
    years: 15,
...
export default function CardPage() {
  ...
  const handleOrderCard = async () => {
    ...
    const toastId = toast.loading(`Achat de la carte ${selectedTier}...`);
    try {
      const res = await fetch("/api/user/card/order", {
        method: "POST",
      ...
      if (res.ok) {
        toast.success("Carte activée !", { id: toastId });
        fetchData();
      } else {
        toast.error(data.error || "Échec de l'achat", { id: toastId });
      }
    } catch (err) {
    ...
    <main className="min-h-screen bg-[#050505] text-white p-6 pb-32 font-sans">
      <header className="max-w-md mx-auto pt-8 mb-10 flex justify-between items-end">
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.4em]">Web3 Hardware Terminal</p>
        </div>
      ...
      <div className="max-w-md mx-auto space-y-8">
        {cardData ? (
          <>
            {/* CARTE ACTIVE */}
            <div className={`w-full aspect-[1.58/1] rounded-[2.5rem] p-8 border ${CARD_TIERS[cardData.type as keyof typeof CARD_TIERS]?.border} bg-gradient-to-br ${CARD_TIERS[cardData.type as keyof typeof CARD_TIERS]?.color} relative overflow-hidden shadow-2xl flex flex-col justify-between transition-all duration-500`}>
          ...
          <div className="space-y-8">
              ...
              {Object.entries(CARD_TIERS).map(([key, tier]) => (
                ...
                >
                  ...
                  <div className="text-left mb-6">
                    ...
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em] italic">Payable en Pi</p>
                  </div>
            ...
            {/* BOX DE CONFIRMATION DE PAIEMENT */}
            <div className="bg-blue-600/10 border border-blue-500/20 p-8 rounded-[3rem] text-center space-y-6">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Montant à payer</p>
                <div className="flex items-center justify-center gap-2">
                  <span className="text-4xl font-black">{(CARD_TIERS[selectedTier].price / PI_RATE).toFixed(2)}</span>
                  <span className="text-2xl font-black italic text-blue-500 text-shadow-glow">π</span>
                </div>
              </div>
              ...
              >
                {actionLoading ? <Loader2 className="animate-spin" size={18} /> : `Activer ${CARD_TIERS[selectedTier].label}`}
              </button>
              <p className="text-[8px] text-slate-600 font-bold uppercase tracking-tight italic">Taux appliqué : 1 π = $0.45</p>
            </div>
          </div>
        )}
              ...
              <div key={tx.id} className="p-4 bg-white/[0.02] border border-white/5 rounded-[22px] flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center">
                    {tx.type === "PAYMENT" ? <ArrowUpFromLine size={16} className="text-blue-400" /> : <ArrowDownToLine size={16} className="text-emerald-400" />}
                  </div>
                  <div>
                ...
                <p className="text-xs font-black">{tx.amount.toFixed(2)} π</p>
              </div>
            )) : (

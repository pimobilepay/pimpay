...
// Configuration des paliers de cartes
const CARD_TIERS = {
...
export default function CardPage() {
  ...
  const [selectedTier, setSelectedTier] = useState<keyof typeof CARD_TIERS>("PREMIUM");
  ...
  const fetchCardData = async () => {
    try {
      ...
      // Récupérer aussi le prix actuel du Pi pour les calculs
    ...
    try {
      const res = await fetch("/api/cards/create", { 
        ...
        body: JSON.stringify({ 
          ...
          holderName: "Utilisateur PimPay" // À lier dynamiquement au profil si besoin
        })
      });
      const data = await res.json();
        ...
        {cardData ? (
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-700">
            ...
            <div className="grid grid-cols-1 gap-4 mt-10">
              ...
              <div className="bg-white/5 border border-white/10 p-5 rounded-[2.5rem] flex items-start gap-4">
                <div className="p-3 bg-white/10 rounded-2xl text-slate-300">
                  <Zap size={20} />
                </div>
                <div>
                  <h4 className="text-sm font-bold text-slate-100">Usage Multi-Devises</h4>
                  <p className="text-[11px] text-slate-400 mt-1 leading-relaxed uppercase tracking-tighter">
                    {cardData.allowedCurrencies?.join(" • ") || "PI ONLY"}
                  </p>
                </div>
              </div>
            </div>
          ...
          /* --- SI L'UTILISATEUR N'A PAS DE CARTE (SÉLECTEUR) --- */
          <div className="space-y-6 animate-in zoom-in-95 duration-500">
            ...
            <div className={`p-8 rounded-[3rem] border ${currentTierInfo.border} bg-gradient-to-br ${currentTierInfo.color} to-transparent text-center relative overflow-hidden`}>
                <div className="relative z-10">
                    <h2 className="text-3xl font-black mb-1 uppercase italic tracking-tighter">Visa {currentTierInfo.label}</h2>
                    ...
                    <ul className="text-left space-y-3 mb-8 max-w-[200px] mx-auto">
                        <li className="flex items-center gap-2 text-[11px] font-medium text-slate-300">
                            <CheckCircle size={14} className="text-blue-500" /> Limite ${currentTierInfo.limit}/j
                        </li>
                        <li className="flex items-center gap-2 text-[11px] font-medium text-slate-300">
                            <CheckCircle size={14} className="text-blue-500" /> Multi-devises
                        </li>
                    </ul>
                    ...
                    <button
                        ...
                        className="w-full h-14 bg-blue-600 text-white font-black uppercase tracking-[0.2em] rounded-2xl flex items-center justify-center gap-3 hover:bg-blue-500 transition-all active:scale-95 disabled:opacity-50 shadow-[0_10px_30px_rgba(37,99,235,0.3)]"
                    >

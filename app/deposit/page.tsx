      ...
      <main className="px-6 mt-6 space-y-6">
        ...
        {activeTab === "mobile" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase ml-1">Opérateur</label><button onClick={() => setIsOperatorModalOpen(true)} className="w-full h-16 bg-slate-900/50 rounded-2xl border border-white/10 px-5 flex items-center justify-between">{selectedOperator ? <div className="flex items-center gap-4"><div className="w-10 h-10 rounded-xl bg-white p-1.5"><img src={selectedOperator.icon} className="w-full h-full object-contain" /></div><span className="text-sm font-black uppercase tracking-tight">{selectedOperator.name}</span></div> : <span className="text-sm font-bold text-slate-500">Choisir</span>}<ChevronDown size={18} className="text-slate-500" /></button></div>
            {selectedOperator && (

  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white flex flex-col">
      <div className="px-6 pt-12 flex items-center gap-4">
        <button onClick={() => router.back()} className="p-3 rounded-2xl bg-slate-900 border border-white/10 active:scale-90 transition-all">
          <ArrowLeft size={20} />
        </button>
        <div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8">
        <div className={`w-full max-w-md space-y-8 ${shake ? "animate-shake" : ""}`}>
          ...
          <div className="flex justify-center gap-4 py-4">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className={`w-4 h-4 rounded-full transition-all duration-300 ${pins[step as 1|2|3].length > i ? "bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.8)] scale-110" : "bg-slate-800 border border-white/10"}`} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900/50 backdrop-blur-3xl border-t border-white/5 rounded-t-[40px] px-10 pt-10 pb-12">
        <div className="grid grid-cols-3 gap-y-6 gap-x-8 max-w-sm mx-auto">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
            <button key={num} type="button" onClick={() => handleNumberPress(num)} className="h-16 text-2xl font-bold rounded-2xl hover:bg-white/5 active:bg-blue-600/20 active:text-blue-500 transition-all">{num}</button>
          ))}
          <div className="flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-slate-700" /></div>
          <button type="button" onClick={() => handleNumberPress(0)} className="h-16 text-2xl font-bold rounded-2xl hover:bg-white/5 active:bg-blue-600/20 active:text-blue-500 transition-all">0</button>
          <button type="button" onClick={deleteDigit} className="h-16 flex items-center justify-center rounded-2xl text-slate-400 hover:text-white active:scale-90 transition-all"><Delete size={24} /></button>
        </div>

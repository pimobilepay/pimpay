...
const UserRow = ({ user, isSelected, onSelect, onUpdateBalance, onResetPassword, onToggleRole, onResetPin, onFreeze, onToggleAutoApprove, onIndividualMaintenance, onViewSessions, onSupport, onBan, onApproveKYC }: any) => {
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-4">
          <div className="relative group" onClick={onSelect}>
             <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-black border uppercase cursor-pointer ${isSelected ? 'bg-blue-600 border-white text-white' : 'bg-slate-800 border-white/5 text-slate-400'}`}>
        ...
        <div className="flex gap-2">
            {user.kycStatus === 'PENDING' && (
                <button onClick={onApproveKYC} className="text-[7px] font-black px-3 py-1 bg-emerald-500 rounded-full text-white uppercase tracking-widest hover:bg-emerald-400 transition-all">
                    Approuver KYC
                </button>
            )}
...
function DashboardContent() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [usersRes, configRes, logsRes, depositsRes, withdrawsRes] = await Promise.all([
      ...
      if (withdrawsRes.ok) setPendingWithdrawals(await withdrawsRes.json());
  ...
  const handleAction = async (userId: string | null, action: string, amount?: number, extraData?: string, userIds?: string[], txId?: string) => {
    try {
      const res = await fetch(`/api/admin`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId, action, amount, extraData, userIds, txId })
      });
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center">
      ...
      <p className="text-blue-500/50 text-[10px] font-black uppercase tracking-[5px]">Initialisation...</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 pb-32 font-sans overflow-x-hidden notranslate" translate="no">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-black text-blue-400 uppercase tracking-[3px]">ADMIN v4.0</span>
            </div>
      ...
      <div className="px-6 space-y-8">
        ...
        {/* CONTENEUR DE CONTENU AVEC KEY UNIQUE POUR RÉPARER REMOVECHILD */}
        <div key={activeTab} className="animate-in fade-in duration-500 transition-all">
            {activeTab === "overview" && (
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <p className="text-[10px] font-black text-white uppercase tracking-widest">Derniers Logs Audit</p>
                </div>
            ...
            <div className="space-y-6">
                <div className="relative group">
                    ...
                    <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-16 bg-slate-900/50 border border-white/5 rounded-[1.5rem] pl-14 pr-6 text-xs font-bold outline-none text-white" placeholder="RECHERCHER..." />
                </div>
                <div className="space-y-4">
                {filteredUsers.map(user => (
                    <UserRow 
                        ...
                        onApproveKYC={() => handleAction(user.id, 'APPROVE_KYC')}
                        onIndividualMaintenance={(u: any) => {
                            const d = prompt("Fin Maint. (YYYY-MM-DD HH:MM):");
                            if(d) handleAction(u.id, "USER_SPECIFIC_MAINTENANCE", 0, d);
                        }}
                    />
                ))}
                </div>
            ...
            {activeTab === "finance" && (
                <div className="space-y-8">
                    <section className="space-y-4">
                        <h2 className="text-[10px] font-black uppercase text-blue-500 flex items-center gap-2 px-2"><ArrowDownLeft size={14}/> Dépôts en attente</h2>
                        {pendingDeposits.map(tx => (
                            <Card key={tx.id} className="bg-slate-900/40 border-white/5 rounded-[2rem] p-6 space-y-4">
                                <div className="flex justify-between items-start">
                                    <p className="text-xs font-black text-white">{tx.fromUser?.name}</p>
                                    <p className="text-xs font-black text-emerald-500">+ {tx.amount} π</p>
                                </div>
                                <div className="p-3 bg-black/40 border border-white/5 rounded-xl font-mono text-[9px] text-blue-300 break-all">{tx.metadata?.txHash}</div>
                                <div className="flex gap-2">
                                    <a href={`https://minepi.com/blockexplorer/tx/${tx.metadata?.txHash}`} target="_blank" className="flex-1 h-12 bg-white/5 rounded-xl flex items-center justify-center gap-2 text-[9px] font-black uppercase"><ExternalLink size={14}/> Explorer</a>
                                    <Button onClick={() => handleAction(null, 'VALIDATE_DEPOSIT', tx.amount, "", [], tx.id)} className="flex-1 bg-emerald-600 rounded-xl font-black text-[9px] uppercase">Valider</Button>
                                </div>
                            </Card>
                        ))}
                    </section>
                    ...
                    <section className="space-y-4">
                        <h2 className="text-[10px] font-black uppercase text-red-500 flex items-center gap-2 px-2"><ArrowUpRight size={14}/> Retraits en attente</h2>
                        {pendingWithdrawals.map(tx => (
                            <Card key={tx.id} className="bg-slate-900/40 border-white/5 rounded-[2rem] p-6 space-y-4">
                                <div className="flex justify-between items-center">
                                    <p className="text-xs font-black text-white">{tx.fromUser?.name}</p>
                                    <p className="text-xs font-black text-red-500">- {tx.amount} π</p>
                                </div>
                                <div className="p-3 bg-black/40 border border-white/5 rounded-xl text-[9px] font-mono text-amber-500">{tx.metadata?.phoneNumber || tx.metadata?.piAddress}</div>
                                <div className="flex gap-2">
                                    <Button onClick={() => handleAction(tx.id, 'REJECT_WITHDRAW')} className="flex-1 bg-white/5 text-red-500 rounded-xl font-black text-[9px] uppercase">Rejeter</Button>
                                    <Button onClick={() => handleAction(tx.id, 'APPROVE_WITHDRAW')} className="flex-1 bg-blue-600 rounded-xl font-black text-[9px] uppercase">Confirmer</Button>
                                </div>
                            </Card>
                        ))}
                    </section>
...
export default function AdminDashboard() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#020617] flex items-center justify-center font-black text-blue-500">CHARGEMENT CORE...</div>}>
      <DashboardContent />
    </Suspense>
  );
}

...
export default function SystemSettings() {
  ...
  const loadData = async () => {
    try {
      ...
      // Extraction de la config et des logs (renvoyés par ton API GET)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        loadData(); // Rafraîchir pour voir le nouveau log
      } else {
  ...
  const handleRollback = async (logId: string) => {
    if (!confirm("Voulez-vous restaurer cet état système ? Cette action est irréversible.")) return;
                ...
                {auditLogs.length > 0 ? auditLogs.map((log) => (
                    <div key={log.id} className="group flex items-center justify-between p-5 bg-black/40 border border-white/5 rounded-[2rem] hover:border-blue-500/30 transition-all">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button onClick={() => handleRollback(log.id)} className="p-3 bg-blue-500/10 text-blue-500 rounded-xl opacity-0 group-hover:opacity-100 hover:bg-blue-600 hover:text-white transition-all">
                                <RotateCcw size={16} />
                            </button>
                        </div>
                    </div>
                )) : (
                    <div className="text-center py-10 text-slate-600 italic text-sm border border-dashed border-white/10 rounded-3xl">
                        No recent core modifications detected.
                    </div>
                )}
            </div>
        </section>

...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white pb-32">
      ...
      <main className="px-6">
        {/* SOLDE (Wallet Model) */}
        <div className="relative w-full aspect-[16/9] bg-gradient-to-br from-blue-600 to-indigo-900 rounded-[32px] p-6 shadow-2xl border border-white/10 mb-8 mt-4">
        ...
        <section>
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-500">Transactions</h3>
            <History size={16} className="text-slate-600" />
          </div>

          <div className="space-y-4">
            <TransactionItem title="Dépôt Mobile Money" type="DEPOSIT" amount="+π 50.00" status="SUCCESS" />
            <TransactionItem title="Transfert vers Utilisateur" type="TRANSFER" amount="-π 25.50" status="SUCCESS" />
            <TransactionItem title="Retrait Mobile Money" type="WITHDRAW" amount="-π 5.00" status="PENDING" />
          </div>
        </section>
      </main>

      <BottomNav onOpenMenu={() => toast.info("Menu ouvert")} />
    </div>
  );
}

function TransactionItem({ title, type, amount, status }: { title: string, type: string, amount: string, status: string }) {
    ...
    <div className="p-4 bg-slate-900/40 border border-white/5 rounded-2xl flex justify-between items-center">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-full w-fit ${status === 'SUCCESS' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
            {status}
          </p>
        </div>
      </div>

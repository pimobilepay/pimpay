 ...
 * lib/proxy.ts
 *
 * Contient toute la logique de routage/protection extraite de l'ancien proxy.ts.
 * Importé et appelé depuis middleware.ts (le seul point d'entrée Next.js).
 */
...
export async function handleProxy(req: NextRequest): Promise<NextResponse> {
  ...
  if (pathname.startsWith("/admin") && !isAdmin) {
    return NextResponse.redirect(new URL(getDestinationByRole(userRole ?? "USER"), req.url));
  }
